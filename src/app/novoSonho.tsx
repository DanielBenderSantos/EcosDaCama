...
const MODOS: Array<{ id: ModoId; emoji: string; label: string }> = [
  { id: "mistico", emoji: "🌙", label: "Místico/Espiritual" },
  { id: "psicologico", emoji: "🧠", label: "Psicológico/Junguiano" },
  { id: "acolhedor", emoji: "💖", label: "Acolhedor/Autoajuda" },
  { id: "historico", emoji: "📜", label: "Histórico/Mítico" },
  { id: "oraculo", emoji: "🔮", label: "Oráculo/Esotérico" },
  { id: "motivacional", emoji: "🌱", label: "Motivacional" },
  { id: "objetivo", emoji: "🌌", label: "Minimalista/Objetivo" },
];
...
export default function NovoSonho() {
  ...
  const [modoInterpretacao, setModoInterpretacao] = useState<ModoId | null>(null);
  ...
  const scrollToInput = (
    ...
    if (Platform.OS === "web") {
      ...
      const node = el?.scrollIntoView ? el : el?._node || el?._inputRef?.current;
      try {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setInterpretacao(s.interpretacao ?? null);
      } catch (e) {
        console.error(e);
  ...
  async function onInterpretarIA() {
    try {
      ...
      if (!modoInterpretacao) {
        alertWebMobile("Escolha o estilo de interpretação primeiro.");
        return;
      }

      setIaLoading(true);

      const txt = await interpretarSonhoIA(
        textoSonho.trim(),
        modoInterpretacao,
        "pt"
      );
            ...
            >
                    ...
                    {step === 0 && (
                      <View>
                        ...
                        <View style={{ gap: 12 }}>
                          ...
                          {/* ESCOLHA DE MODO */}
                          <Text style={{ fontWeight: "700", marginTop: 12, marginBottom: 6, color: COLORS.cinzaEscuro }}>
                            Escolha o estilo da interpretação:
                          </Text>

                          <View style={styles.modosWrap}>
                            {MODOS.map((m) => {
                              const selected = modoInterpretacao === m.id;
                              return (
                                <Pressable
                                  key={m.id}
                                  onPress={() => setModoInterpretacao(m.id)}
                                  style={({ pressed }) => [
                                    styles.modoChip,
                                    {
                                      ...
                                      borderColor: selected ? COLORS.cinzaEscuro : "#d1d5db",
                                ...
                                >
                                  <Text
                                    style={{
                                      fontSize: 13,
                                      fontWeight: "600",
                                      color: selected ? "#0f172a" : COLORS.cinzaEscuro,
                                    }}
                                  >
                                    {m.emoji} {m.label}
                                  </Text>
                            ...
                            <Text style={{ fontWeight: "700", color: "#0f172a" }}>
                              {iaLoading
                                ...
                                : `Interpretar sonho (IA) ✨${
                                    modoInterpretacao
                                      ? ` — ${MODOS.find((m) => m.id === modoInterpretacao)?.label}`
                                      : ""
                                  }`}
                            </Text>
                          </Pressable>
              ...
              <View style={styles.bottomBar}>
                ...
                <ActionButton
                  ...
                  disabled={loading || saving || (step === 1 && humor == null) || (step === 2 && !tipo)}
                  variant="primary"
                />
              </View>
            </View>
